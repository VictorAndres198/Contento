...
function ProfileMenu() {
  ...
  return (
    <Menu open={isMenuOpen} handler={setIsMenuOpen} placement="bottom-end">
      ...
      <MenuList className="shadow-xl shadow-sky-400/10">
        {profileMenuItems.map(({ label, icon }, key) => {
          ...
          return (
            <MenuItem 
              key={label}
              onClick={closeMenu}
              className={`flex items-center gap-2 rounded p-1 px-3 pr-12 text-slate-500 hover:text-slate-800 font-normal bg-white ${
                isLastItem
  ...
  return (
    <React.Fragment>
      <Menu allowHover open={isMenuOpen} handler={setIsMenuOpen}>
        ...
        <MenuList className="hidden w-[36rem] grid-cols-7 gap-3 overflow-visible lg:grid text-slate-800 shadow-xl shadow-sky-400/20">
          <Card
            shadow={false}
            variant="gradient"
            className="col-span-3 grid h-full w-full place-items-center rounded-md bg-sky-500"
          >
...
export default function ComplexNavbar() {
  ...
  return (
    <Navbar className="mx-auto max-w-screen-xl p-2 lg:rounded-full lg:pl-6 mt-6 shadow-xl shadow-sky-400/10">
